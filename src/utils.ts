...
export const useWords = <T>(setLoading: (loading: boolean) => void) => {
    ...
    const loadWords = () =>
        getText('https://words-sordle.iambrian.com/5-letter-words.txt').then((w) => {
            words.push(...w.split('\n'));
        });

    const getRandomWord = async (): Promise<Word> => {
        const word = words[randomNumber(0, 7435)];
        const response: Word[] = await getJson('https://words-sordle.iambrian.com/5-letter-words/' + word + '.json');
        return response[0];
    };
